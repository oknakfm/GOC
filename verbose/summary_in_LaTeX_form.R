...
for(K0 in c(30,50,70)){
  ...
  for(clm_type in c("k-means","k-medoids","GMM_ClusterR","GMM_EII")){
    tmp_table_conv = tmp_table_GOC = NULL
    for(seed_id in 1:10){
      load(paste0(DIR_NAME,"/dataset/preprocessed_seed",seed_id,".RData"))
  ...
  table_LaTeX = NULL
  table_LaTeX = paste0("\\begin{tabular}{llcccc} \n \\toprule \n & & $k$-means & $k$-medoids & GMM~(ClusterR) & GMM~(EII) \\\\ \n \\midrule \n ")
  
  ## NMI for GOC
  table_LaTeX = paste0(table_LaTeX,"\\multirow{2}{*}{NMI} & GOC ")
  for(id in 1:4) table_LaTeX = paste0(table_LaTeX, " & ", table_GOC[1,id])
  table_LaTeX = paste0(table_LaTeX, " \\\\ \n ")
  
  ## NMI for conventional
  table_LaTeX = paste0(table_LaTeX,"& Baseline ")
  for(id in 1:4) table_LaTeX = paste0(table_LaTeX, " & ", table_conv[1,id])
  table_LaTeX = paste0(table_LaTeX, " \\\\ \n \\cmidrule{2-6} \n")
  
  ## F-measure for GOC
  table_LaTeX = paste0(table_LaTeX,"\\multirow{2}{*}{$F$-measure} & GOC ")
  for(id in 1:4) table_LaTeX = paste0(table_LaTeX, " & ", table_GOC[2,id])
  table_LaTeX = paste0(table_LaTeX, " \\\\ \n ")
  
  ## F-measure for conventional
  table_LaTeX = paste0(table_LaTeX,"& Baseline ")
  for(id in 1:4) table_LaTeX = paste0(table_LaTeX, " & ", table_conv[2,id])
  table_LaTeX = paste0(table_LaTeX, " \\\\ \n \\cmidrule{2-6} \n")
  
  ## #clusters for GOC
  table_LaTeX = paste0(table_LaTeX,"$\\#$clusters & GOC ")
  for(id in 1:4) table_LaTeX = paste0(table_LaTeX, " & ", table_GOC[3,id])
  table_LaTeX = paste0(table_LaTeX, " \\\\ \n ")
  
  ## #iterations for GOC
  ## #clusters for GOC
  table_LaTeX = paste0(table_LaTeX,"$\\#$iterations & GOC ")
  for(id in 1:4) table_LaTeX = paste0(table_LaTeX, " & ", table_GOC[4,id])
  table_LaTeX = paste0(table_LaTeX, " \\\\ \n ")
